...
本阶段是**只读诊断**：不发运动或吸盘指令，不改箱体记录，不把测量自动用于抓取。相机的自动移动、增量居中、姿态调整和抓取力确认不在这个面板中实现。
...
图像颜色：绿色为预测顶面及中心，紫色为 SAM mask，黄色为拟合中心及按已知尺寸绘制的矩形，白色十字为图像中心。黄色矩形不是另一次分割，而是拟合中心/yaw 与已知尺寸构成的可视化。
...
- 这是单帧诊断，**没有实现 20 帧稳定确认或自动居中**。若同时运行其他控制面板，本面板不会阻止它们移动机器人。
